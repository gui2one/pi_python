...
class uiItem(object):
    ...
    def getValue(self):
        ...
        if type(self).__name__ == 'Button' and self.parent != None: 
...
class EditText(uiItem):
    ...
    def draw(self):
        ...
        if self.showLabel:
            ...
            offsetX = labelSurface.get_size()[0]+5
            labelPosX = self.rect.left
            labelPosY = self.rect.top  + ( (self.size[1] - labelSurface.get_size()[1])/2.0)
            self.screen.blit(labelSurface,(labelPosX,labelPosY))

        borderRect = pygame.Rect(self.position[0] + offsetX,self.position[1],self.size[0], self.size[1])
        ...
        textPosX = self.rect.left + ( (self.size[0] - surfSize[0])/2.0) + offsetX
